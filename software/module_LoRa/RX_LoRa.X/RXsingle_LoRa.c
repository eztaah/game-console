...
int main(int argc, char** argv) {
    ...
    forever {
        ...
        // wait for valid header reception or for timeout
        reg_val = ReadSXRegister(REG_IRQ_FLAGS);
        while (((reg_val & 0x10) == 0x00) && ((reg_val & 0x80) == 0x00)) {      // wait until Valid Header flag (bit n°4) or Timeout flag (bit n°7) is set
            reg_val = ReadSXRegister(REG_IRQ_FLAGS);
        }
        
        if ((reg_val & 0x10) != 0x00){                          // *********** valid header was received before timeout ***********
            UARTWriteStrLn(" ");
            ...
            UARTWriteStrLn(" ");
            UARTWriteStrLn("nothing received before timeout");
        }

        // check if mode came back to STANDBY
        GetMode();
